...
export default function ProfilePage() {
        ...
        async (candidateId: string) => {
            try {
                ...
                if (availableCommittees.length > 0 && data.interests) {
                    const interestsWithDetails = data.interests.map((interestId: string) => availableCommittees.find((comm) => comm._id === interestId)).filter(Boolean) as Committee[];
    ...
    const fetchRating = useCallback(async () => {
        ...
        try {
            ...
            const isUser = (u: unknown): u is { _id: string; ratings?: Record<string, number> } => typeof u === "object" && u !== null && typeof (u as { _id?: unknown })._id === "string";
            const sid = session.user!.id;
            const current = arr.find((u): u is { _id: string; ratings?: Record<string, number> } => isUser(u) && u._id === sid);
    ...
    const handleTagCommentChange = (tagToUpdate: string, comment: string) => {
        if (candidate && candidate.tags) {
            const newTags = candidate.tags.map((tag) => (tag.tag === tagToUpdate ? { ...tag, comment } : tag));
            setCandidate({ ...candidate, tags: newTags });
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const cleanedAlternateEmails = candidate.alternateEmails.map((email) => email.trim().toLowerCase()).filter((email) => email !== "");
    ...
    return (
        <Container>
            ...
            {isDifferentPhase && (
                <PhaseBanner>
                    <span>
                        Este candidato está en la fase de {candidate.recruitmentPhase}, pero la fase actual es {currentPhase}. Pulsa el botón para cambiar de fase:
                    </span>
            ...
            <Header>
                <ProfileCard>
                    <Avatar src={candidate.photoUrl || defaultAvatar} alt="Foto del candidato" onError={(e) => (e.currentTarget.src = defaultAvatar)} />
                    <InfoGroup>
                        <LabelGroup>
                        ...
                        <ToggleGroup>
                            ...
                            <div style={{ marginTop: -2, display: "flex", alignItems: "center" }}>
                                <Switch id="status-toggle" checked={candidate.active ?? true} disabled={!isEditing} onChange={handleActiveSwitchChange} size="small" />
                                {!candidate.active && <InfoButton onClick={handleInfoButtonClick} iconSize={20} style={{ marginLeft: 5 }} />}
                            </div>
                        </ToggleGroup>
                    </InfoGroup>
                </ProfileCard>
            ...
            <TextField label="URL de Foto" name="photoUrl" value={candidate.photoUrl || ""} onChange={handleInputChange} disabled={!isEditing} fullWidth style={{ marginBottom: isMobile ? 20 : 32 }} />

            {/* Name | Tutor */}
            <TwoColumn>
                <TextField label="Nombre" name="name" value={candidate.name} onChange={handleInputChange} disabled={!isEditing} fullWidth />
                <Autocomplete options={users.map((u) => u.email)} freeSolo value={candidate.tutor} onChange={(_, newValue) => setCandidate({ ...candidate, tutor: newValue || "" })} onInputChange={(_, newInput) => setCandidate({ ...candidate, tutor: newInput })} renderInput={(params) => <TextField {...params} label="Padrino" disabled={!isEditing} fullWidth />} disabled={!isEditing} />
            </TwoColumn>
                    ...
                    {availableTags.map((tagInfo) => {
                        ...
                        return (
                            <TagBadge key={tagInfo.tag}>
                                ...
                                {!isEditing && currentTag && currentTag.comment && !hasNoComment && <TagCommentDisplay>{currentTag.comment}</TagCommentDisplay>}
                            </TagBadge>
                        );
                    })}
                    {!isEditing && (!candidate.tags || candidate.tags.length === 0) && <p>No hay etiquetas para mostrar.</p>}
                </TagsContainer>
            </Section>
            ...
            <TwoColumn>
                <div>
                    ...
                    <EmailRow>
                        <TextField label="Correo principal" value={candidate.email} onChange={(e) => setCandidate({ ...candidate, email: e.target.value })} disabled={!isEditing} fullWidth />
                        <DeleteButton onClick={() => {}} disabled />
                    </EmailRow>
                    {candidate.alternateEmails.map((email, index) => (
                        <EmailRow key={index}>
                            <TextField value={email} onChange={(e) => handleEmailChange(index, e.target.value)} disabled={!isEditing} fullWidth />
                            {isEditing ? <DeleteButton onClick={() => removeEmail(index)} /> : <DeleteButton onClick={() => {}} disabled />}
                        </EmailRow>
                    ))}
                ...
                <div>
                    ...
                    <Autocomplete
                        options={availableCommittees.filter((comm) => !candidateInterests.some((ci) => ci._id === comm._id))}
                        ...
                        value={null}
                        renderInput={(params) => <TextField {...params} label="Añadir interés" fullWidth size="medium" />}
                        disabled={!isEditing}
                        sx={{ mb: 2 }}
                    />
                    <ChipContainer>
                        {candidateInterests.map((interest) => (
                            <InterestBadge key={interest._id} color={interest.color}>
                                {interest.name}
                                {isEditing && <DeleteButton onClick={() => handleRemoveInterest(interest._id)} iconSize={16} needsConfirmation={false} />}
                            </InterestBadge>
                        ))}
                    </ChipContainer>
                </div>
            ...
            <Section>
                ...
                <FormGroup row={!isMobile}>
                    ...
                    <FormControlLabel control={<Checkbox checked={events["Welcome Meeting"] === true} indeterminate={events["Welcome Meeting"] === null} onChange={() => handleEventChange("Welcome Meeting")} disabled={!isEditing} />} label="Welcome Meeting" />
                    <FormControlLabel control={<Checkbox checked={events["Welcome Days"] === true} indeterminate={events["Welcome Days"] === null} onChange={() => handleEventChange("Welcome Days")} disabled={!isEditing} />} label="Welcome Days" />
                    <FormControlLabel control={<Checkbox checked={events["Integration Weekend"] === true} indeterminate={events["Integration Weekend"] === null} onChange={() => handleEventChange("Integration Weekend")} disabled={!isEditing} />} label="Integration Weekend" />
                    <FormControlLabel control={<Checkbox checked={events["Plataforma Local"] === true} indeterminate={events["Plataforma Local"] === null} onChange={() => handleEventChange("Plataforma Local")} disabled={!isEditing} />} label="Plataforma Local" />
                </FormGroup>
            </Section>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <Rating
                            ...
                            sx={{
                                ...
                                "& .MuiRating-iconEmpty": { color: "color-mix(in srgb, var(--button-primary-bg), transparent 70%)" }
                            }}
                            getLabelText={(value) => labels[value as 1 | 2 | 3 | 4 | 5]}
                        />
                        <span style={{ minWidth: 90, fontWeight: 500 }}>{hoverRating ? labels[hoverRating] : rating ? labels[rating] : "Sin selección"}</span>
                    </div>
                    <div style={{ display: "flex", gap: "8px" }}>
                        {[1, 2, 3, 4, 5].map((n) => (
                            <div key={n} style={{ display: "flex", justifyContent: "center", overflow: "visible", width: 48 }}>
                                <small style={{ color: "var(--text-secondary)" }}>{labels[n as 1 | 2 | 3 | 4 | 5]}</small>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <Section>
                ...
                <TextField label="Tus notas privadas sobre este candidato" value={note} onChange={(e) => setNote(e.target.value)} fullWidth multiline rows={4} variant="outlined" />
            ...
            <Section>
                ...
                {formResponses.length > 0 ? formResponses.map((res) => <FormPreview key={res._id} formStructure={res.formId.structure as unknown as string} responses={res.responses} isAccordion={true} />) : <p>No hay respuestas disponibles para este candidato.</p>}
            </Section>
            ...
            <Modal isOpen={showRejectedReasonModal} title="Razón de Rechazo" onClose={!isEditing ? handleRejectedReasonCancel : undefined}>
                <TextField label="Razón de Rechazo" value={tempRejectedReason} onChange={(e) => setTempRejectedReason(e.target.value)} fullWidth multiline rows={4} placeholder="Motivo por el cual el candidato fue rechazado..." variant="outlined" disabled={!isEditing} />
                {isEditing && (
                    <ModalButtons>
