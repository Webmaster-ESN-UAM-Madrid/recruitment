...
const TableRow = styled.div<{
  ...
  background-color: ${(props) =>
    props.$inactive
      ? "#f9f9f9"
      : props.$isDifferentPhase
        ? "var(--table-different-phase-row-bg)"
        : "var(--bg-primary)"};
...
const SelectableTable: React.FC<SelectableTableProps> = ({
  ...
  if (isMobile) {
    ...
    if (isInactiveTable) {
      fixedColumns = [
        ...fixedColumns,
        { key: "rejectedReason", header: "Motivo de Rechazo", fixed: true, width: "1fr" }
      ];
    }
    currentVisibleColumns = [...fixedColumns, ...dynamicColumns];
    currentGridTemplateColumns =
      currentVisibleColumns.map((c) => c.width || "1fr").join(" ") +
      (dynamicColumns.length > 0 ? " 30px" : " 1fr 30px");
  }
        ...
        {loading ? (
          ...
          displayCandidates.map((candidate) => (
            <TableRow
              ...
              $isMobile={isMobile}
              $isDifferentPhase={candidate.recruitmentPhase !== currentPhase}
            >
              {currentVisibleColumns.map((column) => (
                <DataCell key={column.key}>
                  {column.key === "tags" && (
                  ...
                  {column.key === "rejectedReason" && !isMobile && candidate.rejectedReason}{" "}
                  {/* Hide rejectedReason on mobile */}
                  {column.key !== "tags" &&
                    column.key !== "avatar" &&
                    column.key !== "name" &&
          ...
          <ItemCard style={{ border: "none", marginBottom: 0 }}>
...
const DashboardPage: React.FC = () => {
  ...
  const allColumns: Column[] = useMemo(
    () => [
      ...
      { key: "selectedBy", header: "Selecciones de newbies", fixed: false, width: "200px" },
    ...
    if (savedActiveColumns) {
      try {
        ...
        if (
          ...
          parsedColumns.every((item: unknown) => typeof item === "string")
        ) {
          const filtered = (parsedColumns as string[]).filter((id: string) =>
            allColumns.some((c) => c.key === id)
          );
      ...
      try {
        ...
        if (candidatesRes.ok) {
          ...
          const candidatesWithData = candidatesData.map((candidate: Candidate) => {
            ...
            const candidateFeedback = allFeedback[candidate._id] || {
              recruiters: [],
              tutor: [],
              volunteers: []
            };
            ...
            const average =
              ratingCount > 0
                ? ratingEntries.reduce((sum, e) => sum + e.rating, 0) / ratingCount
                : 0;

            const selectionEntries = users
              .filter(
                (u) =>
                  Array.isArray(u.newbieCandidateSelections) &&
                  (u.newbieCandidateSelections as unknown[]).includes(candidate._id)
              )
              .map((u) => ({
                userId: u._id,
                name: u.name || "Reclutador",
            ...
            return {
              ...candidate,
              formResponses: candidateResponses,
        ...
        if (formsRes.ok) {
          ...
          if (formsData.length > 0) {
            ...
            formsData.forEach(
              (form: { _id: string; formIdentifier: string; structure: string }) => {
                const sections = JSON.parse(form.structure).map(
                  ([title, questions]: [string, FormQuestion[]]) => ({
  ...
  useEffect(() => {
    if (!loading && columnsInitialized) {
      localStorage.setItem(
        "dashboardInactiveVisibleColumns",
        JSON.stringify(inactiveVisibleColumnIds)
      );
    }
  ...
  const handleColumnToggle = (
    ...
    const setVisibleIds =
      tableType === "active" ? setActiveVisibleColumnIds : setInactiveVisibleColumnIds;
