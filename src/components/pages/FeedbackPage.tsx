...
const FeedbackPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const [candidatesRes, feedbackRes] = await Promise.all([fetch("/api/candidates/active"), fetch("/api/feedback")]);
    ...
    const handleRowClick = (candidateId: string) => {
        setExpandedRows((prev) => (prev.includes(candidateId) ? prev.filter((id) => id !== candidateId) : [...prev, candidateId]));
    };
                    ...
                    sortedCandidates.map((candidate) => {
                        ...
                        return (
                            <div key={candidate._id}>
                                <TableRow onClick={() => handleRowClick(candidate._id)} $isTutor={candidate.tutor === session?.user?.email} $isMobile={isMobile}>
                                    <DataCell>
                                    ...
                                    <ActionsCell>
                                        {isExpanded ? <HideButton onClick={() => handleRowClick(candidate._id)} ariaLabel="Ocultar comentarios" iconSize={22} /> : <ViewButton onClick={() => handleRowClick(candidate._id)} ariaLabel="Ver comentarios" iconSize={22} disabled={!hasFeedback} />}
                                        <AddButton onClick={() => openModal(candidate)} ariaLabel="Añadir Comentario" showSpinner={false} iconSize={22} />
                                    </ActionsCell>
                                </TableRow>
