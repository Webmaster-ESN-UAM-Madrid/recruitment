...
const CandidateCard = styled.div<{ $hasInterviewResponse?: boolean }>`
  ...
  padding: 8px;
  border: 1px solid
    ${(props) => (props.$hasInterviewResponse ? "var(--brand-primary)" : "var(--border-primary)")};
  border-radius: 16px;
  background-color: var(--bg-primary);
...
const TasksPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [candidatesRes, interviewsRes, usersRes, formsRes, formResponsesRes] =
        await Promise.all([
  ...
  const interviewsWithPendingFeedback = interviews.filter((interview) => {
    ...
    if (
      !session?.user?.id ||
      !interview.interviewers.includes(session.user.id) ||
  ...
  const candidatesWithInterviewResponse = new Set(
    formResponses
      .filter((r) => r.candidateId && interviewFormIds.has(r.formId.toString()))
          ...
          {interviewsWithPendingFeedback.length > 0 ? (
            interviewsWithPendingFeedback.map((interview) => (
              <ItemCard key={interview._id}>
                <p>
                  Tienes feedback pendiente para la entrevista del{" "}
        ...
        <Section>
          ...
          <ItemCard>
            <p style={{ overflowWrap: "anywhere" }}>
              {activeEmails || "No hay emails pendientes."}
            </p>
            {activeEmails && (
              <AcceptButton
                onClick={() =>
                  handleMarkAsSent(activeCandidatesWithPendingEmails.map((c) => c._id))
                }
            ...
            <CandidateCardContainerWrapper>
              <CandidateCardContainer>
                {unscheduledCandidates.map((candidate) => (
                  <CandidateCard
                    key={candidate._id}
                    $hasInterviewResponse={candidatesWithInterviewResponse.has(candidate._id)}
                  >
                    <Avatar
                      src={candidate.photoUrl || defaultAvatar}
                      onError={(e) => (e.currentTarget.src = defaultAvatar)}
                ...
                {interviewsWithUnnotifiedCandidates.map((interview) =>
                  interview.candidates.map((candidateId) => {
                    ...
                    return (
                      !isNotified && (
                        <TableRow
                          key={`${interview._id}-${candidateId}`}
                          gridtemplatecolumns="60px 250px 300px 180px 1fr"
                        >
                          <DataCell>
                            <Avatar
                              src={candidate.photoUrl || defaultAvatar}
                          ...
                          <DataCell>{new Date(interview.date).toLocaleString()}</DataCell>
                          <DataCell>
                            <EditButton onClick={() => openModal(interview)} />
                          </DataCell>
                        </TableRow>
