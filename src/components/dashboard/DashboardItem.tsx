...
const DashboardItem: React.FC<DashboardItemProps> = ({
  ...
  if (isFormQuestion && formResponses) {
    ...
    const rawValue =
      formResponse && formResponse.responses[effectiveKey] !== undefined
        ? formResponse.responses[effectiveKey]
        : "";
    ...
    if (candidate?.interests && candidate.interests.length > 0 && availableCommittees.length > 0) {
      const interestsWithDetails = candidate.interests
        .map((interestId: string) => availableCommittees.find((comm) => comm._id === interestId))
  ...
  if (columnKey === "feedback") {
    const hasFeedback =
      candidate.feedback &&
      (candidate.feedback.recruiters.length > 0 ||
        candidate.feedback.tutor.length > 0 ||
        candidate.feedback.volunteers.length > 0);
    return (
  ...
  if (columnKey === "rating") {
    ...
    return (
      <ItemContainer $isOverflowing={false}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Rating
              ...
              readOnly
              size="small"
              sx={{ color: "var(--button-primary-bg)" }}
            />
            <small>({count})</small>
          </div>
        ...
        {hasRatings && (
          <Modal
            title={`Valoraciones de ${candidate?.name || "[Candidato]"}`}
            isOpen={isModalOpen}
            onClose={() => setIsModalOpen(false)}
          >
              ...
              {candidate.rating?.entries.map((e) => (
                ...
                >
                  ...
                  <img
                    ...
                    src={e.image || "/default-avatar.jpg"}
                    onError={(ev) => (ev.currentTarget.src = "/default-avatar.jpg")}
  ...
  if (columnKey === "selectedBy") {
    const count = candidate.selectedBy?.count || 0;
    const entries = candidate.selectedBy?.entries || [];
    const hasSelections = count > 0;
    const selectionLabel = count === 1 ? "1 voto" : `${count} votos`;
    ...
    return (
      <ItemContainer $isOverflowing={false}>
        ...
        {hasSelections && (
          <Modal
            title={`Personas que seleccionaron a ${candidate?.name || "[Candidato]"}`}
            isOpen={isModalOpen}
            onClose={() => setIsModalOpen(false)}
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {entries.map((entry) => (
                ...
                >
                  ...
                  <img
                    alt="Avatar del reclutador"
                    src={entry.image || "/default-avatar.jpg"}
                    onError={(ev) => (ev.currentTarget.src = "/default-avatar.jpg")}
                    style={{ width: 32, height: 32, borderRadius: "50%", objectFit: "cover" }}
                  />
                  <div style={{ display: "flex", flexDirection: "column" }}>
                    <strong style={{ marginBottom: 2 }}>{entry.name}</strong>
                  </div>
                </div>
              ))}
            </div>
  ...
  if (
    showModalButton ||
    (isFormQuestion &&
      ["MULTIPLE_CHOICE", "CHECKBOX", "GRID", "CHECKBOX_GRID"].includes(question?.type || "") &&
      displayValue === "")
  ) {
