...
function getMappingFromResponse(
...
export const processFormResponse = async (formResponseId: string) => {
  try {
      ...
      if (name && allEmails.length > 0) {
        ...
        if (existingCandidate) {
          ...
          const newEmails = allEmails.filter(
            (e) => e !== existingCandidate.email && !existingCandidate.alternateEmails.includes(e)
          );
    ...
    await createIncident({
      type: "ERROR",
      details:
        `Failed to process form response ${formResponseId}: ` +
...
export const getFormResponses = async () => {
  ...
  try {
    ...
    const formsInCurrentRecruitment = await Form.find({
      recruitmentProcessId: currentRecruitmentId
    }).select("_id");
...
export const getFormResponsesByCandidateId = async (
  candidateId: string
...
export const registerFormConnection = async (
  key: string,
  formData: object,
  code: string,
  appsScriptId: string,
...
export const handleFormResponse = async (
  ...
  try {
    ...
    try {
      ...
      console.error(
        `Error during instant processing of form response ${newFormResponse._id}:`,
        processingError
      );
...
export const validateFormConnection = async (
  key: string,
  code: string,
  formIdentifier?: string,
  canCreateUsers?: boolean
  ...
  try {
    ...
    if (result.status === "failed") {
      return {
        ...
        incidents: result.incidents as IIncident[]
      };
    }
...
export const attachResponseToCandidate = async (responseId: string, candidateId: string) => {
  ...
  try {
    ...
    const respondentEmail = response.respondentEmail.toLowerCase();
    const isNewEmail =
      respondentEmail !== candidate.email.toLowerCase() &&
      !candidate.alternateEmails.map((e: string) => e.toLowerCase()).includes(respondentEmail);
