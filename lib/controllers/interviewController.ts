...
export const getInterviewById = async (id: string): Promise<IInterview | null> => {
  await dbConnect();
  try {
    const interview = await Interview.findById(id).populate("candidates").populate("interviewers");
    return interview;
  } catch (error) {
          ...
          if (existingOpinion) {
            if (opinionUpdate.interviewers) {
              for (const interviewerId in opinionUpdate.interviewers) {
                if (
                  Object.prototype.hasOwnProperty.call(opinionUpdate.interviewers, interviewerId)
                ) {
                  existingOpinion.interviewers.set(
                    interviewerId,
                    opinionUpdate.interviewers[interviewerId]
                  );
                }
              }
            }
