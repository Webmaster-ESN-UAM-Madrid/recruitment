...
const defaultImage =
  "https://img.freepik.com/premium-vector/default-avatar-profile-icon-social-media-user-image-gray-avatar-icon-blank-profile-silhouette-vector-illustration_561158-3383.jpg?semt=ais_items_boosted&w=500";
    ...
    if (oldRecruitmentPhase !== recruitmentPhase) {
      ...
      if (oldRecruitmentPhase === "entrevistas1" && recruitmentPhase === "entrevistas2") {
        ...
        interviews.forEach((interview) => {
          ...
          const opinions =
            interview.opinions instanceof Map
              ? interview.opinions
              : new Map(Object.entries(interview.opinions || {}));
          ...
          opinions.forEach((opinion: any, candidateId: string) => {
            ...
            if (
              opinion?.status === "present" ||
              opinion?.status === "delayed" ||
              opinion?.status === "unset"
            ) {
