...
export const getUserNewbieSelections = async (userId: string) => {
  await dbConnect();
  try {
    const user = await User.findById(userId).select("newbieCandidateSelections newbie");
    if (!user) {
      return { selections: [], newbie: false };
    }
    const selections = Array.isArray(user.newbieCandidateSelections)
      ? user.newbieCandidateSelections
      : [];
    return { selections, newbie: Boolean(user.newbie) };
  } catch (error) {
    console.error("Error fetching user newbie selections:", error);
    return { selections: [], newbie: false };
  }
};

export const updateUserNewbieSelections = async (userId: string, candidateIds: string[]) => {
  await dbConnect();
  try {
    const user = await User.findById(userId);
    if (!user) {
    ...
    user.newbieCandidateSelections = candidateIds;
    await user.save();
  } catch (error) {
    console.error("Error updating newbie selections:", error);
    throw error;
  }
};
